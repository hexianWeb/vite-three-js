...
vec4 getLensDirt(vec2 p){
    ...
    // 基于多圈 smoothstep 插值计算镜头污渍的明暗遮罩
    float dirtMaskA = smoothstep(0.1, 0.0, length(p - vec2(0.25)));
    float dirtMaskB = smoothstep(0.4, 0.0, length(p - vec2(0.75)));
    float dirtMaskC = smoothstep(0.8, 0.0, length(p - vec2(0.875, 0.125)));
    float dirtMask = max(max(dirtMaskA, dirtMaskB), dirtMaskC);
    vec3 o = vec3(mix(0.125, 0.25, dirtMask));
