...
export class MeshReflectorMaterial extends MeshStandardMaterial {
  ...
  onBeforeCompile(shader, ...arguments_) {
    super.onBeforeCompile(shader, ...arguments_);

    if (!shader.defines?.USE_UV) {
      shader.defines.USE_UV = '';
    }

    shader.defines.USE_DEPTH = true;
    shader.defines.USE_BLUR = this.hasBlur;

    let properties = this.reflectorProps;
